...
double extract_results(int, int, int, const char*, double, double);
...
static const int nb_state = 32; //Number of distinct resistance states targeted (they are equally spaced in resistance)
...
static const double compliance_reset = 1E-2; //Compliance current used in DC sweeps in reset operations not necessary but used as a security
static const double meas_time = 5; //Duration in second of the DC sweep
static const int pulse_number = 4; //Number of pulse amplitudes to test for G(V,t) measurements
